...
// listEl.addEventListener('click', onListClick); // 1 подія, яка callback за це відповідає

// function onListClick(e) {
//   console.log((e.currentTarget.style.backgroundColor = 'tomato'));
...
//   e.target.nextElementSibling.style.backgroundColor = 'tomato';
// }
...
createBtn.addEventListener("click", () => {
  ...
  createBtn.after(createBoxes(inputValue));
});
...
// createBtn.after(createBoxes(100));
// console.log(createBoxes(100));
...
const watchedFilmsArr = ["film_2", "film_4", "film_5"];
  ...
  return arr
    .map(
      ({
        ...
        imgUrl,
        id,
      }) => `<li id="${id}"><img src="${imgUrl}" alt="${title}"></li>
`
    )
    .join("");
}
...
const watchedFilms = (arr) => {
  arr.forEach((id) => {
    const film = filmListEl.querySelector(`#${id}`);
    if (film) {
